        ...
        internal override void RunQuery(EvaluationContext ec)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    n = String.Format("Fig_{0}", ec.imagecount);
                    Microsoft.Automata.DirectedGraphs.DotWriter.AutomatonToDot<string>(x => x, aut, n, fname, showName: true);
                }
            }
    ...
    public class DotExpression : Expression
    {
        ...
        internal override void RunQuery(EvaluationContext ec)
        {
            ...
            if (subexpressions[0].IsSFA(ec))
            {
                ...
                {
                    var aut = sfa.Concretize(NrOfElements);
                    Microsoft.Automata.DirectedGraphs.DotWriter.AutomatonToDot<BDD>(ec.solver.CharSetProvider.PrettyPrint, aut, sfa.Name, ec.tw, showName: true, config: new Automata.DirectedGraphs.DotWriter.Config());
                }
            }
            else
